#!/usr/bin/perl
#arguments: -id GMID -m GFS_WCTRL [-o offset_hour | -v valid_datetime] -i verify_interval [-l max_verify_hours] [-d verify_incre_hour]
#sishen 2017-05-18
#----------------------------------
#parse arguments
#----------------------------------
...
if ( ! ("$VALID" and "$GMID" and "$MEMBER" and "$VERIF_INTERVAL")) {
    print "<usage> : $0  -id GMID -m GFS_WCTRL [-v VALIDATE_TIME | -o OFFSET_HOUR] -i VERIF_INTERVAL [-l MAX_VERIF_HOURS] \n";
...
while ($try < $end_try) {
    ...
    #run
    $cmd="$GMODDIR/rtfdda_postproc.verif_SfcObs.CN3.pl -id $GMID -m $MEMBER -c $try -s $start_hour -e $end_hour -d $VERIF_INCRE_HOUR >& $MYLOGDIR/zverif_obssfc.$try";
    print($cmd."\n");
    system($cmd);
    $try=&hh_advan_date($try,1);
}
