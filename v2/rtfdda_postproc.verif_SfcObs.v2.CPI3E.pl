...
for ($h=$START_HOUR; $h<=$END_HOUR; $h=$h+$INCRE_HOUR) {
    ...
    for $isub (1..$n_subdom) {
        ...
        symlink("$ENSPROCS/plot_SFC_and_obs_UPDATED_new.ncl","plot_SFC_and_obs.ncl");
        symlink("$GMODDIR/ensproc/stationlist_site_dom${dom_wrf}","stationlist_site_dom${dom_wrf}");
        symlink("$GMODDIR/ensproc/stationlist_farm_dom${dom_wrf}","stationlist_farm");
        ...
        symlink("$GMODDIR/ensproc/ncl_functions/convert_and_copyout_temp.ncl", "convert_and_copyout.ncl");
...
#        system("test -d $dest || mkdir -p $dest");
        ...
        $ncl = "ncl 'cycle=\"$CYCLE\"' 'file_in=\"$fn\"' 'qcfile_sfc_in=\"$file_obs\"' 'dom=$dom' 'web_dir=\"$WEB_DEST/gifs/\"' 'latlon=\"True\"' 'zoom=\"$iszoom\"' 'lat_s=$DOM_LAT1[$isub-1]' 'lat_e=$DOM_LAT2[$isub-1]' 'lon_s=$DOM_LON1[$isub-1]' 'lon_e=$DOM_LON2[$isub-1]' 'showStats=\"True\"' 'fileStats=\"$file_stats\"' 'optOutput=\"cycleOnly\"' 'filterQCValue=0' plot_SFC_and_obs.ncl >& zout.nclSFC.d${dom}.log";
        print($ncl);
        system($ncl);
        chdir("$WORKDIR");
