#!/usr/bin/perl
#arguments: -id GMID -m GFS_WCTRL [-o offset_hour | -v valid_datetime] -i verify_interval
#sishen 2017-05-18
#may add another optional argument (e.g. -skip), if we don't want every cycle to be verified
#----------------------------------
#parse arguments
#----------------------------------
@argus=@ARGV;
...
if ( ! ("$VALID" and "$GMID" and "$MEMBER" and "$VERIF_INTERVAL")) {
    print "<usage> : $0  -id GMID -m GFS_WCTRL [-v VALIDATE_TIME | -o OFFSET_HOUR] -i VERIF_INTERVAL \n";
...
while ($try < $end_try) {
    ...
    #run
    $cmd="$GMODDIR/rtfdda_postproc.verif_SfcObs.CPI1D.pl -id $GMID -m $MEMBER -c $try -s $start_hour -e $end_hour >& $MYLOGDIR/zverif_obssfc.$try";
    print($cmd."\n");
    system($cmd);
    $try=&hh_advan_date($try,1);
}
